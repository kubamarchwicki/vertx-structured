...
public class StoreRepositoryVerticle extends Verticle {
    final static public String GET_ALL = "store.getAll";
    final static public String SAVE = "store.save";
    final static public String UPDATE = "store.update";
    final static public String DELETE = "store.delete";
    final static private String DB_HANDLER_NAME = "persistor";

    public void start() {

        //deploy persistor
        //build TodoRepository object (which encapsulate the persistor)
        //parse to Todo.class
        //validate (??)
        ...
        container.deployModule("com.bloidonia~mod-jdbc-persistor~2.1.3", persistorCfg, (AsyncResult<String> result) -> {
            container.logger().info("com.bloidonia~mod-jdbc-persistor~2.1.3 deployed? " + (result.succeeded() ? "ok" : result.result()));
        });
